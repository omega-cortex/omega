//! Static translations: headers, labels, and empty-state messages.

/// Look up a header, label, or empty-state key.
pub(super) fn lookup(key: &str, lang: &str) -> Option<&'static str> {
    let v = match key {
        // --- Headers ---
        "status_header" => match lang {
            "Spanish" => "Estado de *OMEGA \u{03a9}*",
            "Portuguese" => "Status do *OMEGA \u{03a9}*",
            "French" => "Statut de *OMEGA \u{03a9}*",
            "German" => "*OMEGA \u{03a9}* Status",
            "Italian" => "Stato di *OMEGA \u{03a9}*",
            "Dutch" => "*OMEGA \u{03a9}* Status",
            "Russian" => "\u{0421}\u{0442}\u{0430}\u{0442}\u{0443}\u{0441} *OMEGA \u{03a9}*",
            _ => "*OMEGA \u{03a9}* Status",
        },
        "your_memory" => match lang {
            "Spanish" => "Tu Memoria",
            "Portuguese" => "Sua Mem\u{00f3}ria",
            "French" => "Votre M\u{00e9}moire",
            "German" => "Dein Ged\u{00e4}chtnis",
            "Italian" => "La Tua Memoria",
            "Dutch" => "Je Geheugen",
            "Russian" => "\u{0422}\u{0432}\u{043e}\u{044f} \u{043f}\u{0430}\u{043c}\u{044f}\u{0442}\u{044c}",
            _ => "Your Memory",
        },
        "recent_conversations" => match lang {
            "Spanish" => "Conversaciones Recientes",
            "Portuguese" => "Conversas Recentes",
            "French" => "Conversations R\u{00e9}centes",
            "German" => "Letzte Gespr\u{00e4}che",
            "Italian" => "Conversazioni Recenti",
            "Dutch" => "Recente Gesprekken",
            "Russian" => "\u{041d}\u{0435}\u{0434}\u{0430}\u{0432}\u{043d}\u{0438}\u{0435} \u{0440}\u{0430}\u{0437}\u{0433}\u{043e}\u{0432}\u{043e}\u{0440}\u{044b}",
            _ => "Recent Conversations",
        },
        "known_facts" => match lang {
            "Spanish" => "Datos Conocidos",
            "Portuguese" => "Fatos Conhecidos",
            "French" => "Faits Connus",
            "German" => "Bekannte Fakten",
            "Italian" => "Fatti Noti",
            "Dutch" => "Bekende Feiten",
            "Russian" => "\u{0418}\u{0437}\u{0432}\u{0435}\u{0441}\u{0442}\u{043d}\u{044b}\u{0435} \u{0444}\u{0430}\u{043a}\u{0442}\u{044b}",
            _ => "Known Facts",
        },
        "scheduled_tasks" => match lang {
            "Spanish" => "Tareas Programadas",
            "Portuguese" => "Tarefas Agendadas",
            "French" => "T\u{00e2}ches Planifi\u{00e9}es",
            "German" => "Geplante Aufgaben",
            "Italian" => "Attivit\u{00e0} Pianificate",
            "Dutch" => "Geplande Taken",
            "Russian" => "\u{0417}\u{0430}\u{043f}\u{043b}\u{0430}\u{043d}\u{0438}\u{0440}\u{043e}\u{0432}\u{0430}\u{043d}\u{043d}\u{044b}\u{0435} \u{0437}\u{0430}\u{0434}\u{0430}\u{0447}\u{0438}",
            _ => "Scheduled Tasks",
        },
        "installed_skills" => match lang {
            "Spanish" => "Skills Instalados",
            "Portuguese" => "Skills Instalados",
            "French" => "Skills Install\u{00e9}s",
            "German" => "Installierte Skills",
            "Italian" => "Skills Installati",
            "Dutch" => "Ge\u{00ef}nstalleerde Skills",
            "Russian" => "\u{0423}\u{0441}\u{0442}\u{0430}\u{043d}\u{043e}\u{0432}\u{043b}\u{0435}\u{043d}\u{043d}\u{044b}\u{0435} \u{043d}\u{0430}\u{0432}\u{044b}\u{043a}\u{0438}",
            _ => "Installed Skills",
        },
        "projects_header" => match lang {
            "Spanish" => "Proyectos",
            "Portuguese" => "Projetos",
            "French" => "Projets",
            "German" => "Projekte",
            "Italian" => "Progetti",
            "Dutch" => "Projecten",
            "Russian" => "\u{041f}\u{0440}\u{043e}\u{0435}\u{043a}\u{0442}\u{044b}",
            _ => "Projects",
        },
        "commands_header" => match lang {
            "Spanish" => "Comandos de *OMEGA \u{03a9}*",
            "Portuguese" => "Comandos do *OMEGA \u{03a9}*",
            "French" => "Commandes de *OMEGA \u{03a9}*",
            "German" => "*OMEGA \u{03a9}* Befehle",
            "Italian" => "Comandi di *OMEGA \u{03a9}*",
            "Dutch" => "*OMEGA \u{03a9}* Commando's",
            "Russian" => "\u{041a}\u{043e}\u{043c}\u{0430}\u{043d}\u{0434}\u{044b} *OMEGA \u{03a9}*",
            _ => "*OMEGA \u{03a9}* Commands",
        },

        "learning_header" => match lang {
            "Spanish" => "Aprendizaje de *OMEGA \u{03a9}*",
            "Portuguese" => "Aprendizado do *OMEGA \u{03a9}*",
            "French" => "Apprentissage de *OMEGA \u{03a9}*",
            "German" => "*OMEGA \u{03a9}* Lernverlauf",
            "Italian" => "Apprendimento di *OMEGA \u{03a9}*",
            "Dutch" => "*OMEGA \u{03a9}* Leerproces",
            "Russian" => "\u{041e}\u{0431}\u{0443}\u{0447}\u{0435}\u{043d}\u{0438}\u{0435} *OMEGA \u{03a9}*",
            _ => "*OMEGA \u{03a9}* Learning",
        },
        "learning_rules" => match lang {
            "Spanish" => "Reglas de comportamiento",
            "Portuguese" => "Regras comportamentais",
            "French" => "R\u{00e8}gles comportementales",
            "German" => "Verhaltensregeln",
            "Italian" => "Regole comportamentali",
            "Dutch" => "Gedragsregels",
            "Russian" => "\u{041f}\u{0440}\u{0430}\u{0432}\u{0438}\u{043b}\u{0430} \u{043f}\u{043e}\u{0432}\u{0435}\u{0434}\u{0435}\u{043d}\u{0438}\u{044f}",
            _ => "Behavioral rules",
        },
        "learning_outcomes" => match lang {
            "Spanish" => "Resultados recientes",
            "Portuguese" => "Resultados recentes",
            "French" => "R\u{00e9}sultats r\u{00e9}cents",
            "German" => "Aktuelle Ergebnisse",
            "Italian" => "Risultati recenti",
            "Dutch" => "Recente resultaten",
            "Russian" => "\u{041d}\u{0435}\u{0434}\u{0430}\u{0432}\u{043d}\u{0438}\u{0435} \u{0440}\u{0435}\u{0437}\u{0443}\u{043b}\u{044c}\u{0442}\u{0430}\u{0442}\u{044b}",
            _ => "Recent outcomes",
        },

        // --- Labels ---
        "uptime" => match lang {
            "Spanish" | "Portuguese" | "French" | "Italian" => "Uptime:",
            "German" => "Laufzeit:",
            "Dutch" => "Uptime:",
            "Russian" => "\u{0410}\u{043f}\u{0442}\u{0430}\u{0439}\u{043c}:",
            _ => "Uptime:",
        },
        "provider" => match lang {
            "Spanish" => "Proveedor:",
            "Portuguese" => "Provedor:",
            "French" => "Fournisseur:",
            "German" => "Anbieter:",
            "Italian" => "Provider:",
            "Dutch" => "Provider:",
            "Russian" => "\u{041f}\u{0440}\u{043e}\u{0432}\u{0430}\u{0439}\u{0434}\u{0435}\u{0440}:",
            _ => "Provider:",
        },
        "database" => match lang {
            "Spanish" => "Base de datos:",
            "Portuguese" => "Banco de dados:",
            "French" => "Base de donn\u{00e9}es:",
            "German" => "Datenbank:",
            "Italian" => "Database:",
            "Dutch" => "Database:",
            "Russian" => "\u{0411}\u{0430}\u{0437}\u{0430} \u{0434}\u{0430}\u{043d}\u{043d}\u{044b}\u{0445}:",
            _ => "Database:",
        },
        "conversations" => match lang {
            "Spanish" => "Conversaciones:",
            "Portuguese" => "Conversas:",
            "French" => "Conversations:",
            "German" => "Gespr\u{00e4}che:",
            "Italian" => "Conversazioni:",
            "Dutch" => "Gesprekken:",
            "Russian" => "\u{0420}\u{0430}\u{0437}\u{0433}\u{043e}\u{0432}\u{043e}\u{0440}\u{044b}:",
            _ => "Conversations:",
        },
        "messages" => match lang {
            "Spanish" => "Mensajes:",
            "Portuguese" => "Mensagens:",
            "French" => "Messages:",
            "German" => "Nachrichten:",
            "Italian" => "Messaggi:",
            "Dutch" => "Berichten:",
            "Russian" => "\u{0421}\u{043e}\u{043e}\u{0431}\u{0449}\u{0435}\u{043d}\u{0438}\u{044f}:",
            _ => "Messages:",
        },
        "facts_label" => match lang {
            "Spanish" => "Datos:",
            "Portuguese" => "Fatos:",
            "French" => "Faits:",
            "German" => "Fakten:",
            "Italian" => "Fatti:",
            "Dutch" => "Feiten:",
            "Russian" => "\u{0424}\u{0430}\u{043a}\u{0442}\u{044b}:",
            _ => "Facts:",
        },
        "due" => match lang {
            "Spanish" => "Vence:",
            "Portuguese" => "Vence:",
            "French" => "\u{00c9}ch\u{00e9}ance:",
            "German" => "F\u{00e4}llig:",
            "Italian" => "Scadenza:",
            "Dutch" => "Vervalt:",
            "Russian" => "\u{0421}\u{0440}\u{043e}\u{043a}:",
            _ => "Due:",
        },
        "language_label" => match lang {
            "Spanish" => "Idioma:",
            "Portuguese" => "Idioma:",
            "French" => "Langue:",
            "German" => "Sprache:",
            "Italian" => "Lingua:",
            "Dutch" => "Taal:",
            "Russian" => "\u{042f}\u{0437}\u{044b}\u{043a}:",
            _ => "Language:",
        },

        // --- Token command ---
        "token_header" => match lang {
            "Spanish" => "Uso de contexto",
            "Portuguese" => "Uso de contexto",
            "French" => "Utilisation du contexte",
            "German" => "Kontextnutzung",
            "Italian" => "Utilizzo contesto",
            "Dutch" => "Contextgebruik",
            "Russian" => "\u{0418}\u{0441}\u{043f}\u{043e}\u{043b}\u{044c}\u{0437}\u{043e}\u{0432}\u{0430}\u{043d}\u{0438}\u{0435} \u{043a}\u{043e}\u{043d}\u{0442}\u{0435}\u{043a}\u{0441}\u{0442}\u{0430}",
            _ => "Context Usage",
        },
        "token_messages" => match lang {
            "Spanish" => "Mensajes:",
            "Portuguese" => "Mensagens:",
            "French" => "Messages:",
            "German" => "Nachrichten:",
            "Italian" => "Messaggi:",
            "Dutch" => "Berichten:",
            "Russian" => "\u{0421}\u{043e}\u{043e}\u{0431}\u{0449}\u{0435}\u{043d}\u{0438}\u{044f}:",
            _ => "Messages:",
        },
        "token_prompt" => match lang {
            "Spanish" => "Prompt del sistema:",
            "Portuguese" => "Prompt do sistema:",
            "French" => "Prompt syst\u{00e8}me:",
            "German" => "System-Prompt:",
            "Italian" => "Prompt di sistema:",
            "Dutch" => "Systeemprompt:",
            "Russian" => "\u{0421}\u{0438}\u{0441}\u{0442}\u{0435}\u{043c}\u{043d}\u{044b}\u{0439} \u{043f}\u{0440}\u{043e}\u{043c}\u{043f}\u{0442}:",
            _ => "System prompt:",
        },
        "token_conversation" => match lang {
            "Spanish" => "Conversaci\u{00f3}n:",
            "Portuguese" => "Conversa:",
            "French" => "Conversation:",
            "German" => "Gespr\u{00e4}ch:",
            "Italian" => "Conversazione:",
            "Dutch" => "Gesprek:",
            "Russian" => "\u{0420}\u{0430}\u{0437}\u{0433}\u{043e}\u{0432}\u{043e}\u{0440}:",
            _ => "Conversation:",
        },
        "token_estimated" => match lang {
            "Spanish" => "Total estimado:",
            "Portuguese" => "Total estimado:",
            "French" => "Total estim\u{00e9}:",
            "German" => "Gesch\u{00e4}tzter Gesamtwert:",
            "Italian" => "Totale stimato:",
            "Dutch" => "Geschat totaal:",
            "Russian" => "\u{041e}\u{0446}\u{0435}\u{043d}\u{043a}\u{0430} \u{0438}\u{0442}\u{043e}\u{0433}\u{043e}:",
            _ => "Estimated total:",
        },
        "token_no_conversation" => match lang {
            "Spanish" => "No hay conversaci\u{00f3}n activa.",
            "Portuguese" => "Nenhuma conversa ativa.",
            "French" => "Pas de conversation active.",
            "German" => "Kein aktives Gespr\u{00e4}ch.",
            "Italian" => "Nessuna conversazione attiva.",
            "Dutch" => "Geen actief gesprek.",
            "Russian" => "\u{041d}\u{0435}\u{0442} \u{0430}\u{043a}\u{0442}\u{0438}\u{0432}\u{043d}\u{043e}\u{0433}\u{043e} \u{0440}\u{0430}\u{0437}\u{0433}\u{043e}\u{0432}\u{043e}\u{0440}\u{0430}.",
            _ => "No active conversation.",
        },

        // --- Context command ---
        "context_header" => match lang {
            "Spanish" => "Contexto del Sistema",
            "Portuguese" => "Contexto do Sistema",
            "French" => "Contexte Syst\u{00e8}me",
            "German" => "Systemkontext",
            "Italian" => "Contesto di Sistema",
            "Dutch" => "Systeemcontext",
            "Russian" => "\u{0421}\u{0438}\u{0441}\u{0442}\u{0435}\u{043c}\u{043d}\u{044b}\u{0439} \u{043a}\u{043e}\u{043d}\u{0442}\u{0435}\u{043a}\u{0441}\u{0442}",
            _ => "System Context",
        },
        "context_on" => match lang {
            "Spanish" | "Portuguese" | "French" | "Italian" => "on",
            "German" => "an",
            "Dutch" => "aan",
            "Russian" => "\u{0432}\u{043a}\u{043b}",
            _ => "on",
        },
        "context_off" => match lang {
            "German" => "aus",
            "Dutch" => "uit",
            "Russian" => "\u{0432}\u{044b}\u{043a}\u{043b}",
            _ => "off",
        },
        "context_total" => match lang {
            "Spanish" => "Total estimado:",
            "Portuguese" => "Total estimado:",
            "French" => "Total estim\u{00e9}:",
            "German" => "Gesch\u{00e4}tzter Gesamtwert:",
            "Italian" => "Totale stimato:",
            "Dutch" => "Geschat totaal:",
            "Russian" => "\u{041e}\u{0446}\u{0435}\u{043d}\u{043a}\u{0430} \u{0438}\u{0442}\u{043e}\u{0433}\u{043e}:",
            _ => "Estimated total:",
        },
        "context_tip" => match lang {
            "Spanish" => "Las secciones se activan seg\u{00fa}n palabras clave.\nUsa /context full para ver el prompt completo.",
            "Portuguese" => "As se\u{00e7}\u{00f5}es s\u{00e3}o ativadas por palavras-chave.\nUse /context full para ver o prompt completo.",
            "French" => "Les sections s'activent par mots-cl\u{00e9}s.\nUtilisez /context full pour voir le prompt complet.",
            "German" => "Sektionen werden per Schl\u{00fc}sselw\u{00f6}rter aktiviert.\nNutze /context full f\u{00fc}r den vollst\u{00e4}ndigen Prompt.",
            "Italian" => "Le sezioni si attivano per parole chiave.\nUsa /context full per vedere il prompt completo.",
            "Dutch" => "Secties worden geactiveerd door sleutelwoorden.\nGebruik /context full voor de volledige prompt.",
            "Russian" => "\u{0420}\u{0430}\u{0437}\u{0434}\u{0435}\u{043b}\u{044b} \u{0430}\u{043a}\u{0442}\u{0438}\u{0432}\u{0438}\u{0440}\u{0443}\u{044e}\u{0442}\u{0441}\u{044f} \u{043f}\u{043e} \u{043a}\u{043b}\u{044e}\u{0447}\u{0435}\u{0432}\u{044b}\u{043c} \u{0441}\u{043b}\u{043e}\u{0432}\u{0430}\u{043c}.\n\u{0418}\u{0441}\u{043f}\u{043e}\u{043b}\u{044c}\u{0437}\u{0443}\u{0439}\u{0442}\u{0435} /context full \u{0434}\u{043b}\u{044f} \u{043f}\u{043e}\u{043b}\u{043d}\u{043e}\u{0433}\u{043e} \u{043f}\u{0440}\u{043e}\u{043c}\u{043f}\u{0442}\u{0430}.",
            _ => "Sections activate based on message keywords.\nUse /context full to see the raw prompt.",
        },

        // --- Empty states ---
        "no_pending_tasks" => match lang {
            "Spanish" => "Sin tareas pendientes.",
            "Portuguese" => "Sem tarefas pendentes.",
            "French" => "Aucune t\u{00e2}che en attente.",
            "German" => "Keine anstehenden Aufgaben.",
            "Italian" => "Nessuna attivit\u{00e0} in sospeso.",
            "Dutch" => "Geen openstaande taken.",
            "Russian" => "\u{041d}\u{0435}\u{0442} \u{043e}\u{0436}\u{0438}\u{0434}\u{0430}\u{044e}\u{0449}\u{0438}\u{0445} \u{0437}\u{0430}\u{0434}\u{0430}\u{0447}.",
            _ => "No pending tasks.",
        },
        "no_facts" => match lang {
            "Spanish" => "A\u{00fa}n no hay datos almacenados.",
            "Portuguese" => "Nenhum fato armazenado ainda.",
            "French" => "Aucun fait enregistr\u{00e9} pour l'instant.",
            "German" => "Noch keine Fakten gespeichert.",
            "Italian" => "Nessun fatto memorizzato.",
            "Dutch" => "Nog geen feiten opgeslagen.",
            "Russian" => "\u{041f}\u{043e}\u{043a}\u{0430} \u{043d}\u{0435}\u{0442} \u{0441}\u{043e}\u{0445}\u{0440}\u{0430}\u{043d}\u{0451}\u{043d}\u{043d}\u{044b}\u{0445} \u{0444}\u{0430}\u{043a}\u{0442}\u{043e}\u{0432}.",
            _ => "No facts stored yet.",
        },
        "no_history" => match lang {
            "Spanish" => "A\u{00fa}n no hay historial de conversaciones.",
            "Portuguese" => "Ainda n\u{00e3}o h\u{00e1} hist\u{00f3}rico de conversas.",
            "French" => "Pas encore d'historique de conversation.",
            "German" => "Noch kein Gespr\u{00e4}chsverlauf.",
            "Italian" => "Nessuna cronologia ancora.",
            "Dutch" => "Nog geen gespreksgeschiedenis.",
            "Russian" => "\u{0418}\u{0441}\u{0442}\u{043e}\u{0440}\u{0438}\u{0438} \u{0440}\u{0430}\u{0437}\u{0433}\u{043e}\u{0432}\u{043e}\u{0440}\u{043e}\u{0432} \u{043f}\u{043e}\u{043a}\u{0430} \u{043d}\u{0435}\u{0442}.",
            _ => "No conversation history yet.",
        },
        "no_learning" => match lang {
            "Spanish" => "A\u{00fa}n no hay datos de aprendizaje. Interact\u{00fa}a conmigo para que aprenda.",
            "Portuguese" => "Ainda n\u{00e3}o h\u{00e1} dados de aprendizado. Interaja comigo para eu aprender.",
            "French" => "Pas encore de donn\u{00e9}es d'apprentissage. Interagissez avec moi pour que j'apprenne.",
            "German" => "Noch keine Lerndaten. Interagiere mit mir, damit ich lerne.",
            "Italian" => "Nessun dato di apprendimento. Interagisci con me per farmi imparare.",
            "Dutch" => "Nog geen leerdata. Communiceer met me zodat ik kan leren.",
            "Russian" => "\u{0414}\u{0430}\u{043d}\u{043d}\u{044b}\u{0445} \u{043e}\u{0431}\u{0443}\u{0447}\u{0435}\u{043d}\u{0438}\u{044f} \u{043f}\u{043e}\u{043a}\u{0430} \u{043d}\u{0435}\u{0442}. \u{041e}\u{0431}\u{0449}\u{0430}\u{0439}\u{0442}\u{0435}\u{0441}\u{044c} \u{0441}\u{043e} \u{043c}\u{043d}\u{043e}\u{0439}, \u{0447}\u{0442}\u{043e}\u{0431}\u{044b} \u{044f} \u{0443}\u{0447}\u{0438}\u{043b}\u{0441}\u{044f}.",
            _ => "No learning data yet. Interact with me so I can learn.",
        },
        "no_skills" => match lang {
            "Spanish" => "No hay skills instalados. Crea un directorio en ~/.omega/skills/ con un archivo SKILL.md.",
            "Portuguese" => "Nenhum skill instalado. Crie um diret\u{00f3}rio em ~/.omega/skills/ com um arquivo SKILL.md.",
            "French" => "Aucun skill install\u{00e9}. Cr\u{00e9}ez un r\u{00e9}pertoire dans ~/.omega/skills/ avec un fichier SKILL.md.",
            "German" => "Keine Skills installiert. Erstelle ein Verzeichnis in ~/.omega/skills/ mit einer SKILL.md-Datei.",
            "Italian" => "Nessuno skill installato. Crea una cartella in ~/.omega/skills/ con un file SKILL.md.",
            "Dutch" => "Geen skills ge\u{00ef}nstalleerd. Maak een map in ~/.omega/skills/ met een SKILL.md bestand.",
            "Russian" => "\u{041d}\u{0430}\u{0432}\u{044b}\u{043a}\u{0438} \u{043d}\u{0435} \u{0443}\u{0441}\u{0442}\u{0430}\u{043d}\u{043e}\u{0432}\u{043b}\u{0435}\u{043d}\u{044b}. \u{0421}\u{043e}\u{0437}\u{0434}\u{0430}\u{0439}\u{0442}\u{0435} \u{043a}\u{0430}\u{0442}\u{0430}\u{043b}\u{043e}\u{0433} \u{0432} ~/.omega/skills/ \u{0441} \u{0444}\u{0430}\u{0439}\u{043b}\u{043e}\u{043c} SKILL.md.",
            _ => "No skills installed. Create a directory in ~/.omega/skills/ with a SKILL.md file.",
        },
        "no_projects" => match lang {
            "Spanish" => "No se encontraron proyectos. Crea carpetas en ~/.omega/projects/ con ROLE.md",
            "Portuguese" => "Nenhum projeto encontrado. Crie pastas em ~/.omega/projects/ com ROLE.md",
            "French" => "Aucun projet trouv\u{00e9}. Cr\u{00e9}ez des dossiers dans ~/.omega/projects/ avec ROLE.md",
            "German" => "Keine Projekte gefunden. Erstelle Ordner in ~/.omega/projects/ mit ROLE.md",
            "Italian" => "Nessun progetto trovato. Crea cartelle in ~/.omega/projects/ con ROLE.md",
            "Dutch" => "Geen projecten gevonden. Maak mappen in ~/.omega/projects/ met ROLE.md",
            "Russian" => "\u{041f}\u{0440}\u{043e}\u{0435}\u{043a}\u{0442}\u{044b} \u{043d}\u{0435} \u{043d}\u{0430}\u{0439}\u{0434}\u{0435}\u{043d}\u{044b}. \u{0421}\u{043e}\u{0437}\u{0434}\u{0430}\u{0439}\u{0442}\u{0435} \u{043f}\u{0430}\u{043f}\u{043a}\u{0438} \u{0432} ~/.omega/projects/ \u{0441} \u{0444}\u{0430}\u{0439}\u{043b}\u{043e}\u{043c} ROLE.md",
            _ => "No projects found. Create folders in ~/.omega/projects/ with ROLE.md",
        },

        _ => return None,
    };
    Some(v)
}
