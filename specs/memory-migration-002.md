...
This enforces a closed enum at the database level. The three permitted values correspond to the `AuditStatus` Rust enum in `backend/crates/omega-memory/src/audit.rs`:
...
The audit log is written by `AuditLogger` in `backend/crates/omega-memory/src/audit.rs`. The `log()` method:
...
The gateway (`backend/src/gateway.rs`) calls `AuditLogger::log()` after every interaction, regardless of outcome:
...
| Aspect | Detail |
|--------|--------|
| File | `backend/crates/omega-memory/migrations/002_audit_log.sql` |
...
| Rust writer | `AuditLogger` in `backend/crates/omega-memory/src/audit.rs` |
| Lines of SQL | 20 |
